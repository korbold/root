    ...
    <div class="page-header">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal">
            ...
            <li class="nav-item">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="{{route('admin.vendor.view', ['restaurant'=>$restaurant->id, 'tab'=> 'discount'])}}"  aria-disabled="true">{{__('discounts')}}</a>
                </li>
                <li class="nav-item">
        ...
        <div class="card-header py-2">
            <div class="search--button-wrapper">
                <h5 class="card-title">
                    ...
                    <span>{{__('messages.discount')}} {{__('messages.info')}}</span>
                </h5>
            ...
            <div class="row gy-3">
                <div class="col-md-4 align-self-center text-center">
                    <div class="discount-item text-center">
                        <h5 class="subtitle">{{__('messages.discount')}} {{__('messages.amount')}}</h5>
                        <h4 class="amount">{{$restaurant->discount?round($restaurant->discount->discount):0}}%</h4>
                    </div>
                </div>
                <div class="col-md-4 text-center text-md-left">
                    <div class="discount-item">
                        <h5 class="subtitle">{{__('messages.duration')}}</h5>
                        <ul class="list-unstyled list-unstyled-py-3 text-dark">
                            <li class="p-0 pt-1 justify-content-center justify-content-md-start">
                                <span>{{__('messages.start')}} {{__('messages.date')}} :</span> <strong>{{$restaurant->discount?date('Y-m-d',strtotime($restaurant->discount->start_date)):''}} {{$restaurant->discount?date(config('timeformat'), strtotime($restaurant->discount->start_time)):''}}</strong></li>
                            <li class="p-0 pt-1 justify-content-center justify-content-md-start">
                                <span>{{__('messages.end')}} {{__('messages.date')}} :</span> <strong>{{$restaurant->discount?date('Y-m-d', strtotime($restaurant->discount->end_date)):''}} {{$restaurant->discount?date(config('timeformat'), strtotime($restaurant->discount->end_time)):''}}</strong></li>
                        </ul>
                    </div>
                </div>
                <div class="col-md-4 text-center text-md-left">
                    <div class="discount-item style-2">
                        <h5 class="subtitle">{{__('messages.purchase')}} {{translate('messages.conditions')}}</h5>
                        <ul class="list-unstyled list-unstyled-py-3 text-dark">
                            <li class="p-0 pt-1 justify-content-center justify-content-md-start">
                                <span>{{__('messages.max')}} {{__('messages.purchase')}} {{__('messages.discount')}} :</span> <strong>{{\App\CentralLogics\Helpers::format_currency($restaurant->discount?$restaurant->discount->max_discount:0)}}</strong></li>
                            <li class="p-0 pt-1 justify-content-center justify-content-md-start">
                                <span>{{__('messages.min')}} {{__('messages.purchase')}} {{__('messages.amount')}} :</span> <strong>{{\App\CentralLogics\Helpers::format_currency($restaurant->discount?$restaurant->discount->min_purchase:0)}}</strong></li>
                        </ul>
                    </div>
                </div>
            </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header pb-3 shadow">
        <h4 class="modal-title m-0" id="exampleModalCenterTitle">{{$restaurant->discount?__('messages.update'):__('messages.add')}} {{__('messages.discount')}}</h4>
        ...
        <form action="{{route('admin.vendor.discount',[$restaurant['id']])}}" method="post" id="discount-form">
            ...
            <div class="row gx-2">
                <div class="col-md-4 col-6">
                    <div class="form-group">
                        <label class="form-label font-medium text-capitalize" for="title">{{__('messages.discount')}} {{translate('messages.amount')}} (%)</label>
                ...
                <div class="col-md-4 col-6">
                    <div class="form-group">
                        <label class="form-label font-medium text-capitalize" for="title">{{__('messages.max')}} {{__('messages.discount')}} ({{\App\CentralLogics\Helpers::currency_symbol()}})</label>
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-6">
                    <div class="form-group">
                        <label class="form-label font-medium text-capitalize" for="title">{{__('messages.end')}} {{__('messages.date')}}</label>
                ...
                <div class="col-md-6 col-6">
                    <div class="form-group">
                        <label class="form-label font-medium text-capitalize" for="title">{{__('messages.start')}} {{__('messages.time')}}</label>
                ...
                <div class="col-md-6 col-6">
                    <label class="form-label font-medium text-capitalize" for="title">{{__('messages.end')}} {{__('messages.time')}}</label>
            ...
            <div class="form-group text-right mb-0">
                @if($restaurant->discount)
                <button type="reset" class="btn btn--reset mr-2 h--37px">{{__('messages.reset')}}</button>
                @endif
                <button type="submit" class="btn btn--primary h--37px">{{$restaurant->discount?__('messages.update'):__('messages.add')}}</button>
            </div>
        </form>
